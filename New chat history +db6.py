...
# ✅ Chat context from the database (passed as a variable)
chat_context = [
    {"user": d["message"], "assistant": d["response"]}
    for d in database_data  # Replace `database_data` with actual data from DB
]
...
# ✅ Function to retrieve relevant past context from FAISS
...
# ✅ Function to format the last few stored conversations from `chat_context`
def format_chat_history(chat_context, max_turns=5):
    """Convert the last N chat interactions into a formatted string."""
    recent_context = chat_context[:max_turns]  # Take the most recent messages
...
while True:
    ...
    # Load memory and format database-based chat history
    ...
    # ✅ Append new message to `chat_context`
    new_entry = {"user": question, "assistant": response_text}
    chat_context.insert(0, new_entry)  # Add newest exchange to the front
