...
# Create FAISS Vector Store from past messages using Hugging Face embeddings
def create_vector_store(docs):
    # Extract embeddings for the documents using the Hugging Face model
...
def update_vector_store(vector_store, new_message):
    ...
    embedding = embedding_model.encode(new_message["content"])
    
    # Add the new document chunks to FAISS
    vector_store.add_documents(split_doc, embeddings=[embedding])
